...
int main(int nargs, char *args[])
{
    ...
    for (i = 0; i < parts; i++)
    {
        qargs.val = &valors[i * (ndades / parts)];
        qargs.ne = ndades / parts;
        pthread_create(&threads[i], NULL, qs_aux, (void *)&qargs);        
    }
    ...
    for (i = 1; i < parts/2; i *= 2)
    {
        ...
        {
            margs.val = &valors[j * (ndades / parts)];

            if (i == 1)
                margs.n = 2 * (ndades / parts);
            else
                margs.n = 2 * i * (ndades / parts);

            margs.vo = &valors2[j * (ndades / parts)];
            pthread_create(&threads[j], NULL, merge_aux, (void *)&margs);   
        }
    }
